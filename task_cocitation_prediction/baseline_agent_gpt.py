"""GPT-5 + tool-using agent baseline for future-influential co-citation prediction.

For each evaluation instance, runs an OpenAI Chat Completions agent loop with five
leakage-safe corpus tools. The target paper's title, abstract, and key references
are part of the user message (causally available per the task spec). The agent
emits a final `Reasoning: ... \\nPredictions: [corpus_id, ...]` block; the harness
parses and saves the corpus_id list, padded to k=1000 with random pre-cutoff papers.

Run:
    python3 -m task_cocitation_prediction.baseline_agent_gpt \\
        --split test \\
        --embeddings_dir data/corpus/test \\
        --model gpt-5-mini \\
        --max_instances 100 \\
        --num_workers 16 \\
        --eval_instances_cache data/task_cocitation_prediction/test/eval_instances.json \\
        --output_dir data/task_cocitation_prediction/test/predictions
"""
...
from task_cocitation_prediction.agent_tools import (
...
def _build_user_message(instance: Dict, papers_dict: Dict) -> str:
    ...
    topic_labels = target_paper.get("topic_labels") or []
    categories = target_paper.get("categories") or []

    ref_blocks = []
    for rid in instance.get("key_reference_ids") or []:
        ref_paper = papers_dict.get(rid)
        if ref_paper is None:
            continue
        ref_blocks.append(
            f"--- Reference (corpus_id={rid}) ---\n"
            f"Title: {(ref_paper.get('title') or '').strip()}\n"
            f"Abstract: {(ref_paper.get('abstract') or '').strip()[:500]}"
        )
    ...
    return (
        ...
        f"Target paper's key references ({len(instance.get('key_reference_ids') or [])} refs):\n\n"
        + ("\n\n".join(ref_blocks) if ref_blocks else "(none)")
        + "\n\nPredict the corpus_ids of papers most likely to be CO-CITED with the target paper "
        "in the upcoming months — i.e., papers that future researchers will cite alongside the target. "
        "Use the tools to traverse the cocitation graph (which past papers cite the target's references "
        "alongside other related work?) and explore topical neighborhoods. Aim for ~100-200 corpus_ids."
    )
...
def run_agent(instance, papers_dict, corpus, gritlm, client, model, system_prompt, max_rounds):
    ...
    target_corpus_id = instance["corpus_id"]
    target_paper = papers_dict.get(target_corpus_id, {})
    # For cocitation: target text IS shown to the LLM (in the user message); the leakage filter
    # blocks the target paper itself plus any near-duplicate (title or 50-char abstract substring).
    ctx = ToolContext(
        corpus=corpus,
    ...
    for round_idx in range(max_rounds + 1):
        ...
        if round_idx == max_rounds:
            ...
            break
    return {"final_text": final_text, "trace": trace, "ctx": ctx}
...
def main():
    parser = argparse.ArgumentParser(description="GPT-5 + tools agent baseline for cocitation prediction")
    ...
    parser.add_argument("--eval_months", type=int, default=4)
    parser.add_argument("--lookahead_months", type=int, default=8)
    parser.add_argument("--eval_instances_cache", type=str, default=None)
    ...
    parser.add_argument("--prompt_template", type=str, default="task_cocitation_prediction/templates/agent_cocitation.prompt")
    parser.add_argument("--output_suffix", type=str, default="")
    args = parser.parse_args()

    utils.log("Loading corpus + GRIT embeddings...")
    all_papers, _, all_embeddings = utils.load_corpus(
    ...
    evaluation_instances = create_evaluation_instances(all_papers, papers_dict, eval_months=args.eval_months, lookahead_months=args.lookahead_months, cache_path=args.eval_instances_cache)
    ...
    def _worker(item):
        ...
        predicted_ids, hallucinated = _parse_predictions(out["final_text"], out["ctx"].candidate_ids)
        predicted_ids = _pad_predictions(predicted_ids, args.k, papers_dict, instance["date"], [instance["corpus_id"]])
    ...
    with cf.ThreadPoolExecutor(max_workers=args.num_workers) as ex:
        ...
        for fut in tqdm(cf.as_completed(futures), total=len(futures), desc="Agent cocitation"):
            rec = fut.result()
