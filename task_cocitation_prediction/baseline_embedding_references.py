"""Embedding-references baseline for co-citation prediction. Ranks past papers by similarity to mean-pooled embedding of P's key_references."""
...
def predict_cocited(key_reference_ids, k, index, all_embeddings, distance_metric):
    """Predict co-cited papers by querying FAISS with the mean-pooled embedding of P's key_references."""
    reference_embeddings = [get_embedding(all_embeddings, cid) for cid in key_reference_ids if cid in all_embeddings]
    if len(reference_embeddings) == 0:
        return [], []
    query_embedding = utils.aggregate_embeddings(reference_embeddings, distance_metric)
...
def main():
    parser = argparse.ArgumentParser(description="Embedding-references baseline for co-citation prediction")
    ...
    output_path = os.path.join(args.output_dir, f"predictions.embedding_references.{args.embedding_type}.json")
    ...
    utils.log(f"Running embedding-references baseline with embedding: {args.embedding_type}")
    predictions = []
    pending_adds = []
    for paper in tqdm(postdated_papers, desc="Running embedding-references baseline"):
        ...
        if corpus_id in eval_instance_dict:
            ...
            if idx in selected_indices:
                predicted_ids, predicted_scores = predict_cocited(instance["key_reference_ids"], args.k, index, all_embeddings, distance_metric)
