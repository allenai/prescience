"""Author-momentum baseline for topic-pair growth prediction (causally safe, inner-split calibrated).

For each paper labeled with both topics in the pair, sums a per-author metric
(num_papers / h_index / num_citations at paper date) across its authors. Per-pair score = sum over
labeled papers.

Calibration: rate = Σ_p N_p(F_inner) / (Σ_p Score_p(H_inner) × |F_inner|)
Prediction: predicted(p) = Score_p(H) × rate × |F|
"""
...
def compute_pair_scores(membership, papers_by_id, period_start, period_end, metric):
    """Sum author scores across papers in [period_start, period_end) per pair."""
    ...
    for pair_id, corpus_ids in membership.items():
        for cid in corpus_ids:
            ...
            if not (period_start <= p["date"] < period_end):
                continue
            scores[pair_id] += author_score(p, metric)
    return scores
...
def main():
    ...
    inner_scores = compute_pair_scores(membership, papers_by_id, history_start, inner_history_end, args.author_metric)
    full_scores = compute_pair_scores(membership, papers_by_id, history_start, history_end, args.author_metric)
