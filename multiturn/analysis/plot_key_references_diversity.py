"""Plot cumulative key_references diversity comparing natural vs synthetic corpora."""
import os
import argparse
...
def main():
    parser = argparse.ArgumentParser(description="Plot cumulative key_references diversity.")
    parser.add_argument("--natural_path", type=str, required=True, help="Path to natural diversity JSON")
    parser.add_argument("--synthetic_paths", type=str, nargs="+", required=True, help="Paths to synthetic diversity JSONs")
    parser.add_argument("--output_path", type=str, default="figures/multiturn/key_references_diversity.png", help="Output figure path")
    ...
    metric_label = "Effective # of Cited Papers" if args.metric == "effective" else "Unique Cited Papers"
    plt.xlabel("Date", fontsize=8)
    plt.ylabel(metric_label, fontsize=8)
    plt.title("Cumulative Prior Work Diversity", fontsize=9, fontweight="bold")
