"""Compute cumulative author diversity over time for a single corpus."""
import os
import math
import random
import argparse
...
def main():
    ...
    for bucket in tqdm(sorted(target_buckets.keys()), desc="Processing buckets"):
        ...
        # Subsample to match reference bucket sizes (does nothing on synthetic corpus)
        ...
        # Update cumulative author counts
        for paper in bucket_papers:
            for author in paper.get("authors", []):
                author_id = author["author_id"]
                if author_id not in cumulative_counts:
                    cumulative_counts[author_id] = 0
                cumulative_counts[author_id] += 1
            total_papers += 1

        unique_count = len(cumulative_counts)
        effective_count = math.exp(compute_entropy(cumulative_counts))
        total_appearances = sum(cumulative_counts.values())
