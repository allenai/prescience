"""R-Precision decomposition by reference familiarity for prior work prediction baselines."""
import os
import argparse
...
FAMILIARITY_BUCKETS = [(0, 0), (1, 1), (2, 3), (4, float("inf"))]
...
def compute_reference_familiarity(author_ids, reference_id, cutoff_date, sd2publications, all_papers_dict):
    """Count prior citations of reference_id by any author before cutoff_date."""
    count = 0
    for author_id in author_ids:
        if author_id not in sd2publications:
            continue
        for pub_id in sd2publications[author_id]:
            paper = all_papers_dict[pub_id]
            if paper["date"] >= cutoff_date:
                break
            if "key_references" in paper and paper["key_references"]:
                for ref in paper["key_references"]:
                    if ref["corpus_id"] == reference_id:
                        count += 1
                        break
    return count
...
def main():
    parser = argparse.ArgumentParser(description="Analyze R-Precision by reference familiarity")
    ...
    for corpus_id in tqdm(common_ids, desc="Processing instances"):
        ...
        for baseline in baseline_predictions:
            ...
            for gt_id in gt_reference_ids:
                familiarity = compute_reference_familiarity(author_ids, gt_id, cutoff_date, sd2publications, all_papers_dict)
    ...
    plt.xlabel("Prior Citations", fontsize=8)
