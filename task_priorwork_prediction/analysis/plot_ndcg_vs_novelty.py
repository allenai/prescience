"""nDCG vs reference novelty analysis for prior work prediction baselines."""
import os
import argparse
...
def get_prior_citations(author_ids, cutoff_date, sd2publications, all_papers_dict):
    """Get set of papers cited by any author before cutoff_date."""
    citations = set()
    for author_id in author_ids:
        ...
        for pub_id in prior_pubs:
            ...
            if "key_references" in paper and paper["key_references"] is not None:
                for ref in paper["key_references"]:
                    citations.add(ref["corpus_id"])
    return citations


def compute_novelty_fraction(author_ids, gt_reference_ids, cutoff_date, sd2publications, all_papers_dict):
    """Compute fraction of GT references that are new (never cited by any author before)."""
    if len(gt_reference_ids) == 0:
        return 0.0
    prior_citations = get_prior_citations(author_ids, cutoff_date, sd2publications, all_papers_dict)
    new_count = sum(1 for ref_id in gt_reference_ids if ref_id not in prior_citations)
    return new_count / len(gt_reference_ids)


def main():
    parser = argparse.ArgumentParser(description="Analyze nDCG vs reference novelty")
    ...
    for corpus_id in tqdm(common_ids, desc="Computing novelty"):
        ...
        novelty = compute_novelty_fraction(author_ids, gt_reference_ids, cutoff_date, sd2publications, all_papers_dict)
    ...
    plt.xlabel("Reference Novelty Fraction", fontsize=8)
    plt.ylabel("Mean nDCG", fontsize=8)
    plt.title("nDCG vs Reference Novelty", fontsize=9, fontweight="bold")
