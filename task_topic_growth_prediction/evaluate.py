"""Evaluation script for topic-growth prediction.

Computes four metric blocks:

1. **absolute**: per-topic predicted_growth vs gt_growth as raw counts.
2. **share**: normalize each side by its sum across topics; removes uniform corpus-pipeline-lag confounds and calibration effects.
3. **delta_share**: subtract history_share from both predicted_share and gt_share so the trivial "no change" baseline gets R² ≈ 0; R² > 0 means the baseline predicts real topic dynamics beyond the size prior.
4. **ratio**: divide both sides by history_share; multiplicative analog to delta_share that emphasizes proportional growth (more sensitive to small topics that double in share than to large topics that grow by 5%). Filters out topics absent from history.
...
def compute_aggregate_metrics(results, history_share_by_id=None):
    ...
    if history_share_by_id is not None:
        ...
        # ratio over topics with history_share > 0
...
def history_share_from_topics(topics_path):
    """Load topics file, compute history_share per cluster_id."""
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate topic growth predictions (absolute, share, delta_share, ratio)")
    parser.add_argument("--predictions_path", type=str, required=True, help="Predictions JSON path")
    parser.add_argument("--topics_path", type=str, default=None, help="Topics benchmark JSON path; required for delta_share and ratio metrics")
    parser.add_argument("--output_dir", type=str, default="data/task_topic_growth_prediction/scored", help="Output directory")
    ...
    if args.topics_path:
        utils.log(f"Loading topics benchmark from {args.topics_path} for history_share lookup")
        history_share_by_id = history_share_from_topics(args.topics_path)
        utils.log(f"history_share computed for {len(history_share_by_id)} topics; sum={sum(history_share_by_id.values()):.4f}")
