"""Leakage-safe tool implementations for the priorwork prediction agent baseline.

Five tools are exposed to the agent (search_papers, get_paper, get_author_recent_papers,
get_references_of, get_papers_citing). All tool results pass through `_is_leaking`,
which drops the target paper itself, any post-cutoff paper, any paper whose title
matches the target's, and any paper whose abstract overlaps the target's by 50 chars.
The agent never sees the target's content.
"""
...
TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "each with corpus_id, title, abstract, date, and authors. Useful for understanding what an "
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "title, abstract, date, and authors. References that are themselves post-cutoff or "
                "hidden are filtered out. Returns null if the paper is unknown or hidden."
            ),
            "parameters": {
                "type": "object",
                "properties": {"corpus_id": {"type": "string"}},
    ...
    },
    {
        "type": "function",
        "function": {
            "name": "get_papers_citing",
            "description": (
                "Get pre-cutoff papers that cite a given paper as a KEY (highly influential) reference. "
                "Useful for traversing the citation graph backwards (i.e., who builds on this work). "
                "Returns up to `limit` papers (default 10, max 50)."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "corpus_id": {"type": "string", "description": "The corpus_id whose citers we want to find."},
                    "limit": {"type": "integer", "default": 10}
                },
                "required": ["corpus_id"]
            }
...
class ToolContext:
    ...
    def _is_leaking(self, paper: Dict) -> bool:
        ...
        if self.target_corpus_id is not None and cid == self.target_corpus_id:
            return True
...
def search_papers(query: str, ctx: ToolContext, limit: int = 10) -> List[Dict]:
    ...
    over = limit * 5
    q_emb = ctx.gritlm.embed_query(query)
...
def get_paper(corpus_id, ctx: ToolContext) -> Optional[Dict]:
    paper = ctx.corpus.papers_dict.get(corpus_id)
    if paper is None:
        return None
...
def get_references_of(corpus_id, ctx: ToolContext) -> Optional[List[Dict]]:
    paper = ctx.corpus.papers_dict.get(corpus_id)
    if paper is None:
        return None
...
def get_papers_citing(corpus_id, ctx: ToolContext, limit: int = 10) -> List[Dict]:
    """Find pre-cutoff papers whose key_references include `corpus_id`."""
    limit = max(1, min(int(limit), 50))
    citers = ctx.corpus.cited_by.get(corpus_id, [])
    out = []
    for cid in citers:
        if cid not in ctx.candidate_ids:
            continue
        paper = ctx.corpus.papers_dict.get(cid)
        if paper is None:
            continue
...
def dispatch_tool_call(tool_name: str, arguments_json: str, ctx: ToolContext) -> str:
    ...
    elif tool_name == "get_papers_citing":
        result = get_papers_citing(args.get("corpus_id"), ctx, limit=args.get("limit", 10))
    else:
...
class CorpusView:
    """Read-only shared corpus state (papers_dict, sd2pubs, FAISS index, citation inverted index)."""

    def __init__(self, papers_dict: Dict, sd2publications: Dict, faiss_index, faiss_id_map: List, cited_by: Dict):
        self.papers_dict = papers_dict
        self.sd2publications = sd2publications
        self.faiss_index = faiss_index
        self.faiss_id_map = faiss_id_map
        self.cited_by = cited_by  # ref_corpus_id -> [citer corpus_ids]
...
def build_cited_by_index(papers_iter):
    """Build inverted index: ref_corpus_id -> list of citer corpus_ids. Iterates `papers_iter` once."""
    cited_by = {}
    for paper in papers_iter:
        ...
        for r in paper.get("key_references") or []:
            ...
            if rid is None:
                continue
            cited_by.setdefault(rid, []).append(cid)
    return cited_by
