"""nDCG vs first author experience analysis for coauthor prediction baselines."""
import os
import argparse
...
def main():
    parser = argparse.ArgumentParser(description="Analyze nDCG vs first author experience")
    ...
    # Compute experience for each common instance
    utils.log("Computing first author experience")
    instance_experience = {}
    for corpus_id in tqdm(common_ids, desc="Computing experience"):
        ...
        experience = get_experience(first_author_id, cutoff_date, sd2publications, all_papers_dict)
    ...
    plt.xlabel("First Author Experience (# Papers)", fontsize=8)
    plt.ylabel("Mean nDCG", fontsize=8)
    plt.title("nDCG vs First Author Experience", fontsize=9, fontweight="bold")
